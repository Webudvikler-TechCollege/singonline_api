...
songController.get(`/${url}/:id`, Authorize, async (req: Request, res: Response) => {
...
songController.post(`/${url}`, Authorize, async (req: Request, res: Response) => {
...
songController.put(`/${url}/:id`, Authorize, async (req: Request, res: Response) => {
    const id = parseInt(req.params.id, 10);
    if(isNaN(id)) {
        res.status(400).json({ message: "Invalid song ID" });
    }
...
songController.delete(`/${url}/:id`, Authorize, async (req: Request, res: Response) => {
    const id = parseInt(req.params.id, 10);
    if(isNaN(id)) {
        res.status(400).json({ message: "Invalid song ID" });
    }
