...
const url = 'artists';
artistController.get(`/${url}`, Authorize, async (req, res) => {
    try {
...
artistController.get(`/${url}/:id`, Authorize, async (req, res) => {
...
artistController.post(`/${url}`, Authorize, async (req, res) => {
    const { name } = req.body;
    try {
...
artistController.put(`/${url}/:id`, Authorize, async (req, res) => {
    const id = parseInt(req.params.id, 10);
    if (isNaN(id)) {
        res.status(400).json({ message: "Invalid Artist ID" });
    }
...
artistController.delete(`/${url}/:id`, Authorize, async (req, res) => {
    const id = parseInt(req.params.id, 10);
    if (isNaN(id)) {
        res.status(400).json({ message: "Invalid Artist ID" });
    }
